#!/bin/bash
#SBATCH --job-name=nddr_anuros
#SBATCH --output=logs/nddr_%j.out
#SBATCH --error=logs/nddr_%j.err
...
# Para forzar orden: sbatch --dependency=afterok:<CMLC_JOB_ID> NDDR_khipu.sh
# Si no alcanzan las 8h: ./submit_chain.sh NDDR_khipu.sh nddr
...
echo "Ejecutando NDDR MTL | Job ID: $SLURM_JOB_ID | Nodo: $(hostname)"
nvidia-smi --query-gpu=name,memory.total,memory.used --format=csv

# NDDR MTL: K ramas conv+recurrentes independientes fusionadas por capas NDDR.
# Es la arquitectura mas pesada de las 4 (escala mal con K); batch_size mas
# chico (32) como margen de seguridad de memoria.
python3 train.py \
    --system nddr \
    --data_dir "$WORKDIR" \

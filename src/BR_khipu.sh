#!/bin/bash
#SBATCH --job-name=br_anuros
#SBATCH --output=logs/br_%j.out
#SBATCH --error=logs/br_%j.err
...
# Para forzar orden: sbatch --dependency=afterok:<CMLC_JOB_ID> BR_khipu.sh
# Si no alcanzan las 8h: ./submit_chain.sh BR_khipu.sh br
...
echo "Ejecutando BR | Job ID: $SLURM_JOB_ID | Nodo: $(hostname)"
nvidia-smi --query-gpu=name,memory.total,memory.used --format=csv

# BR entrena un modelo INDEPENDIENTE por clase y por fold (5 clases x 5 folds
# = 25 entrenamientos), a diferencia de CMLC/HPS/NDDR que entrenan uno conjunto.
python3 train.py \
    --system br \
    --data_dir "$WORKDIR" \
